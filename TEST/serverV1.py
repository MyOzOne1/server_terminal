...
# Настройка 1С вход
app.config['ONE_C_USERNAME'] = 'базуеввв'  # Логин администратора 1С
app.config['ONE_C_PASSWORD'] = 'gjkrjdybr'  # Пароль администратора 1С
app.config['ONE_C_PAYSLIP_URL'] = 'http://192.168.202.6/test3/hs/LK_GBS/EmployeeData/PaySlip' # запрос на получение расчетного листа
app.config['ONE_C_VACATIONS_URL'] = 'http://192.168.202.6/test3/hs/LK_GBS/EmployeeData/Vacations' #запрос на получение данных об отпусках
...
class SafeLogger:
    ...
    def error(self, msg, *args, **kwargs):
        ...
        safe_args = [arg.encode('utf-8', 'replace').decode('utf-8', 'replace') 
                    if isinstance(arg, str) else arg for arg in args]
        self.logger.error(safe_msg, *safe_args, **kwargs)
...
def verify_code():
    try:
        ...
        session['user_id'] = user['id']  # Теперь user - словарь
                ...
                if response.status_code == 200:
                    ...
                    if vacations_info.get('is_json', False):
                        try:
                            ...
                            # Извлекаем дату отпуска из JSON
                    ...
                    if not user_data['vacation_dates']:
                        if content:
...
def get_vacations_info():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if response.status_code == 200:
                # Используем контент как есть, без предположений о кодировке
                ...
                # Проверяем, является ли ответ base64
                try:
                    # Если это валидный base64 - декодируем
        ...
        safe_logger.error("Error getting vacations info: %s", str(e))
        return jsonify({'error': 'Internal server error'}), 500
    finally:
        if conn:
            release_db_connection(conn)
