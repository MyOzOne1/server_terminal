...
app.config.update(
    ...
    PERMANENT_SESSION_LIFETIME=timedelta(hours=2),  # Увеличиваем время жизни
    SESSION_REFRESH_EACH_REQUEST=True,
    SESSION_TYPE='filesystem',  # Явно указываем тип сессии
    SESSION_FILE_DIR='./flask_sessions',  # Директория для хранения сессий
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                ADD COLUMN IF NOT EXISTS access_history TEXT DEFAULT ''  -- Изменено с JSONB на TEXT, по умолчанию пустая строка
            """)
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session:  # Проверяем, что пользователь залогинен (user_id в сессии)
            return redirect(url_for('login'))  # Перенаправляем на /login, если нет
...
def check_session():
    ...
    public_endpoints = ['login', 'verify_code', 'resend_code', 'static', 'api_profile_content', 'api_sync_session']
...
def update_session_activity():
    """Обновление времени активности сессии и удаление флага logged_out"""
    if 'user_id' in session:
        session['last_activity'] = datetime.now().isoformat()
        # Удаляем флаг logged_out если пользователь авторизован
        if 'logged_out' in session:
            session.pop('logged_out', None)

@app.route('/profile_tab')
def profile_tab():
    """Endpoint для управления вкладкой профиля в клиентском приложении"""
    ...
    if 'user_id' in session and 'sid' in session:
        ...
        if 'last_activity' in session:
            ...
            if (datetime.now() - last_activity).total_seconds() <= app.config['PERMANENT_SESSION_LIFETIME'].total_seconds():
                ...
                if card_code:
                    # Используем существующую логику рендеринга профиля
                    return profile(card_code)
                else:
                        ...
                        try:
                            with conn.cursor() as cursor:
                                ...
                                if pass_record:
                                    ...
                                    return profile(pass_record['code'])
                        except Exception as e:
                            logger.error("Error fetching card code: %s", e)
                        finally:
                            release_db_connection(conn)
    
    # Если сессии нет или она истекла - загружаем страницу логина
    return render_template('login_by_card.html')
...
def login_by_card():
    card_id = request.args.get('cardId')
    # Используем другой endpoint по умолчанию, который не требует card_code
    return_url = request.args.get('return_url', url_for('purchase'))  # Изменили на 'purchase'
...
def verify_code():
    try:
        ...
        logger.debug("Received card_code: '%s', code: '%s', sid: '%s', channel: '%s'", 
                    card_code, user_code, sid, channel)
        ...
        if code_valid:
            ...
            try:
                ...
                # УДАЛЯЕМ ФЛАГ logged_out ЕСЛИ ОН ЕСТЬ
                if 'logged_out' in session:
                    session.pop('logged_out', None)
                ...
                is_qt_client = 'qt' in user_agent or 'darina-qt-client' in user_agent
                
                if is_qt_client:
                    # Для Qt клиента возвращаем JSON с информацией об успешной авторизации
                    response_data = {
                        'success': True,
                        'sid': session_id,
                        'card_code': card_code,
                    ...
                    # Устанавливаем куку вручную для Qt клиента
                    ...
                    profile_url = url_for('profile', card_code=card_code)
                    ...
                    logger.debug(f"Setting session cookie for web client: {session_id}")
                    return response
...
@app.route('/api/profile_content')
def api_profile_content():
    logger.info("=== API_PROFILE_CONTENT CALLED ===")
    logger.info(f"Session keys: {list(session.keys())}")
    logger.info(f"Cookies from request: {request.cookies}")
    
    # Проверяем сессию по user_id и sid
    if 'user_id' in session and 'sid' in session:
        # Проверяем таймаут сессии
        if 'last_activity' in session:
            try:
                ...
                if time_diff <= app.config['PERMANENT_SESSION_LIFETIME'].total_seconds():
                    # Сессия активна - обновляем время активности
                    session['last_activity'] = datetime.now().isoformat()
                    session.modified = True
                    ...
                    logger.info(f"User authenticated: user_id={user_id}, card_code={card_code}")
                    
                    if card_code:
                        return render_template('profile_1.html', 
                                            user={'card_code': card_code, 'id': user_id})
                    else:
                        # Если card_code нет, ищем его
                        conn = get_db_connection()
                        if conn:
                            try:
                                with conn.cursor() as cursor:
                                    ...
                                    if pass_record:
                                        session['card_code'] = pass_record['code']
                                        session.modified = True
                                        logger.info(f"Found card code: {pass_record['code']}")
                                        return render_template('profile_1.html', 
                                                            user={'card_code': pass_record['code'], 'id': user_id})
                            except Exception as e:
                                logger.error(f"Error fetching card code: {e}")
                            finally:
                                release_db_connection(conn)
                
                else:
                    logger.warning(f"Session expired - time difference: {time_diff} seconds")
                    session.clear()
                    return render_template('login_by_card.html')
    ...
    # Если сессии нет или она истекла - возвращаем страницу логина
    logger.info("No active session, returning login page")
    return render_template('login_by_card.html')

@app.route('/api/update_activity', methods=['POST'])
def api_update_activity():
    """Принудительное обновление активности сессии"""
    if 'user_id' in session:
        session['last_activity'] = datetime.now().isoformat()
        session.modified = True
        logger.info(f"Activity updated for user_id: {session['user_id']}")
    ...
    try:
        ...
        try:
            ...
            response = requests.get(
                'http://192.168.210.201:5050/api/get_personal_data',
                cookies=cookies,
                timeout=10
            )
        ...
        try:
            cookies = {'session': request.cookies.get('session')}
            response = requests.get(
                'http://192.168.210.201:5050/api/get_vacations_info',
                cookies=cookies,
                timeout=100
            )
        ...
        return render_template('profile_1.html', user=user_data)
    except Exception as e:
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if response.status_code == 200:
                ...
                # Парсим данные и создаем структурированный ответ
                try:
...
def parse_1c_vacation_data(content):
    ...
    try:
        ...
        if content.strip().startswith('{') or content.strip().startswith('['):
            data = json.loads(content)
            # Адаптируйте под вашу структуру JSON от 1С
        ...
        # Если данные в HTML/текстовом формате
        else:
            # Используйте BeautifulSoup для парсинга HTML
            from bs4 import BeautifulSoup
            soup = BeautifulSoup(content, 'html.parser')
            
            # Пример парсинга HTML таблицы
            ...
            for table in tables:
                ...
                for row in rows:
                    ...
                    # Ищем данные по ключевым словам
...
def logout():
    ...
    user_agent = request.headers.get('User-Agent', '').lower()
    if 'qt' in user_agent:
        logger.info("Qt-клиент: возвращаем JSON для загрузки /login_by_card")
        server_url = request.host_url.rstrip('/')
        redirect_url = f"{server_url}/login_by_card"
        return jsonify({"success": True, "redirect_url": redirect_url})
    else:
        logger.info("Web-клиент: перенаправляем на /login")
        return redirect(url_for('login'))
...
#________________________________________________________________________________________________
# API для получения новой продукции
...
def api_new_products():
    ...
    # Заглушка - в реальности здесь будет запрос к другому серверу или базе данных
    new_products = [
        {
            "id": 1,
            "product": "Газовая PL GM241 008 1W",
            "description": "Артикул: 4607152049183",
        ...
        {
            "id": 2,
            "product": "Индукционная PLS ECI241 616 1S",
            "description": "Артикул: 4607152049589",
            "arrival_date": "20.01.2024",
            "amount": 3,
            "price": 47290
        },
        {
            "id": 3,
            "product": "Комбинированная 1F8 2312 At",
            "description": "Артикул: 4607152046090",
        ...
        {
            "id": 4,
            "product": "Газовая 0D1 G12005 W",
            "description": "Артикул: 464607152049596",
            "arrival_date": "12.01.2024",
            "amount": 8,
            "price": 22690
        },
        {
            "id": 5,
            "product": "Электрическая 1D EM141 407 W",
            "description": "Артикул: 4607152044140",
            "arrival_date": "18.01.2024",
            "amount": 4,
            "price": 28590
        }
    ]
...
# API для получения уцененной продукции
...
def api_discount_products():
    ...
    discount_products = [
        {
            "id": 101,
            "product": "Газовая PL GM241 008 1W (уценка)",
            "description": "Небольшие косметические повреждения корпуса",
            "discount_date": "22.01.2024",
            "amount": 2,
            "price": 45000
        },
        {
            "id": 102,
            "product": "Электрическая 1D EM141 407 W",
            "description": "Уценен из-за царапины на подставке",
            "discount_date": "19.01.2024",
            "amount": 1,
            "price": 28590
        },
        {
            "id": 103,
            "product": "Электрическая 1B EC331 606 At",
            "description": "Без оригинальной упаковки",
            "discount_date": "14.01.2024",
            "amount": 3,
            "price": 27490
        },
        {
            "id": 104,
            "product": "Комбинированная 1B KM441 301 W",
            "description": "Работает исправно, внешний вид удовлетворительный",
            "discount_date": "16.01.2024",
            "amount": 5,
            "price": 21490
        }
    ]
...
def log_access():
    ...
    # Не логируем на статичных и некоторых API эндпоинтах
    if request.endpoint in ['static', 'api_profile', 'api_access_history']:
        return

    # Фильтр ботов по User-Agent
    user_agent = request.headers.get('User-Agent', '').lower()
    if 'python-requests' in user_agent or 'bot' in user_agent or 'spider' in user_agent:
        return

    # Логируем только один раз на сессию
    if session.get('first_access_logged', False):
        return

    now = datetime.now()

    # Фильтр по частоте — не чаще 30 секунд
    if 'last_logged' in session:
        try:
            last_time = datetime.fromisoformat(session['last_logged'])
            if (now - last_time) < timedelta(seconds=30):
                return
        except Exception:
            # В случае ошибки парсинга времени — игнорируем фильтр
            pass
    session['last_logged'] = now.isoformat()
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                ...
                WHERE card_code = %s AND (user_id IS NULL OR user_id = %s)
            """, (session['card_code'], session['user_id']))
            result = cursor.fetchone()

            # Получаем историю доступа или создаем пустой список
            if result and result['access_history']:
                history = result['access_history']
            ...
            if result:
                cursor.execute("""
                    ...
                    WHERE card_code = %s
                """, (json.dumps(history), session['user_id'], session['card_code']))
            else:
                cursor.execute("""
                    INSERT INTO telegram_bindings (card_code, user_id, access_history)
                    VALUES (%s, %s, %s)
                """, (session['card_code'], session['user_id'], json.dumps([new_entry])))
            conn.commit()

            logger.debug(f"Access logged: {new_entry}")

            # После успешного логирования ставим флаг, чтобы не логировать повторно
            session['first_access_logged'] = True
...
def api_access_history():
    from datetime import datetime  # Импорт для сортировки (если еще не добавлен глобально)
        ...
        with conn.cursor() as cursor:
            ...
            if isinstance(history_raw, str):
                try:
                    ...
                    logger.error("Cannot parse access_history JSON: %s", str(e))
                    history = []
            ...
            # Сортировка от новых к старым (по полю 'date')
            if history:
                def sort_key(entry):
                    try:
                        return datetime.fromisoformat(entry['date'])
                    except (ValueError, KeyError):
                        # Если дата некорректна, ставим самое раннее значение для сортировки
                        return datetime.min
                
                history = sorted(history, key=sort_key, reverse=True)
            
            logger.info(f"History fetched and sorted: {len(history)} entries")
            return jsonify(history)  # Возвращаем отсортированный список объектов истории
    except Exception as e:
