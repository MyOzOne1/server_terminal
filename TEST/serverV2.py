...
def login():
    if request.method == 'GET':
        # Для веб-клиента: отображаем страницу входа
        return render_template('login.html')
    
    if request.method == 'POST':
        # Для POST-запросов от любого клиента
        ...
        if send_telegram_code(card_code):
            ...
            if is_web_client:
                # Для веб-клиента: отображаем страницу ввода кода
...
def verify_code():
    try:
        ...
        if not card_code or not user_code:
            ...
            return jsonify({
                'success': False,
                'error': 'Не указаны код карты или код подтверждения'
            }), 400
        ...
        try:
            ...
            # Генерируем уникальный идентификатор сессии (SID)
            session_id = str(uuid.uuid4())
            
            # Обновление сессии Flask
            ...
            if is_web_client:
                # Для веб-клиента: перенаправляем на страницу профиля
                response = redirect(url_for('profile', card_code=card_code))
                response.set_cookie(
                    'session',
                    value=session_id,
                    httponly=True,
                ...
                # Для PySide клиента: возвращаем JSON с данными
                return jsonify({
                    'success': True,
                    'sid': session_id,
                    'card_code': card_code,
                    'redirect_url': url_for('profile', card_code=card_code)
                })
...
def api_profile():
    ...
    if 'sid' not in session or session['sid'] != request.args.get('sid'):
...
def profile(card_code):
    # Убрана очистка сессии при получении sid
    if 'sid' in request.args:
        # Вместо очистки просто обновляем sid
        session['sid'] = request.args['sid']
        session.permanent = True
        logger.info(f"Updated session SID: {session['sid']}")
    
    # Проверка валидности сессии
    if 'sid' not in session or 'user_id' not in session:
        logger.warning("Session ID or user_id missing, redirecting to login")
        return redirect(url_for('login'))
